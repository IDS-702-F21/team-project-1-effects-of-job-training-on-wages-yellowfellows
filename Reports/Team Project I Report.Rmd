...
None of which is significant, so not inlcuding these in the model. (Answered question 2)
...
What is a likely range for the effect of training?
confidence interval, includes 0, so effect could either be postiive or negative, so we can't conclude anything.

Is there any evidence that the effects differ by demographic groups?
see above

Are there other interesting associations with positive wages that are worth mentioning?
Age is significant to the model. 

Age is significant in model, 

Final: age_int
conclusion: treatment is insignificant as predictor, and useless
...
part 1 and part 2 are consistent, age plays an influential part in both model. 

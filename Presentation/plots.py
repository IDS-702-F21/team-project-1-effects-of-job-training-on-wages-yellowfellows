...
df["race"] = np.select(
    [df.black == 1, df.hispan == 1,], ["black", "hispan"], default="other"
)
...
ax.hist(df["re78"], bins=30, ec="k", color="#03529B")
...
#%%
########################## Logistic Regression Pred Plot ####################################
df = df.assign(re78dummy=(df.re78 > 0).astype("int"))
mod = smf.logit(
    formula="re78dummy ~ age + treat + married + black + re74 + treat:age", data=df
)
res = mod.fit()
print(res.summary())
...
for black in (0, 1):
    for treated in (0, 1):
        pdp_data_points = []
        for x in ages:
            copydf = df["age treat married black re74 re78dummy".split()].copy()
            copydf["age"] = np.full_like(df.age, fill_value=x)
            copydf["treat"] = np.full_like(df.shape[0], fill_value=treated)
            copydf["black"] = np.full_like(df.shape[0], fill_value=black)
            preds = res.predict(copydf)
            pdp_data_points.append(preds.mean())
        result_mtx[f"black_{black}+treated_{treated}"] = pdp_data_points
result_df = pd.DataFrame(result_mtx, index=ages)


config = {
    "black_0+treated_1": dict(color="#03529B", annot="Treated + Not Black", y_offset=0),
    "black_1+treated_1": dict(color="#061953", annot="Treated + Black", y_offset=0),
    "black_0+treated_0": dict(color="red", annot="Not Treated + Not Black", y_offset=0),
    "black_1+treated_0": dict(color="darkred", annot="Not Treated + Black", y_offset=0),
}
...
for key, val in config.items():
    ...
    ax.text(
        s=val["annot"],
        x=56,
        y=result_df[key].values[-1] + val["y_offset"],
        va="center",
        color=val["color"],
        size=14,
        weight="bold",
...
sns.despine()
ax.axhline(0.5, ls="--", color="0.5", zorder=0)

ax.set_xlabel("Age (years)", size=18)
ax.set_ylabel("Predicted probability of non-zero income", size=18)
ax.set_title("Predicted Probability for Age by Black", size=20, weight="bold")


plt.tight_layout()
plt.savefig("prediction_plot_logreg.png", dpi=300, facecolor="white")
